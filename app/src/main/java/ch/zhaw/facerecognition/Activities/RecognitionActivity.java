...
public class RecognitionActivity extends Activity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if(images == null || images.size() == 0 || faces == null || faces.length == 0 || ! (images.size() == faces.length)){
            ...
            for(int i = 0; i<faces.length; i++){
                ...
                Log.d(TAG, "hello: "+rec.recognize(images.get(i), ""));
                khem = rec.recognize(images.get(i), "");
                Log.d(TAG, "hello1: "+khem);
                name1(khem);
    ...
    private void name1(String khem) {
        ...
        db.collection("temporary").document("name")
                .set(city)
    ...
    public void onResume()
    {
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                ...
                Log.d(TAG, "run: sex"+sharedPref.getString("key_classification_method", getResources().getString(R.string.eigenfaces)));
                handler.post(new Runnable() {
                    @Override
                    public void run() {
